...
#### 图片代理 TMDB 插件必须要 json 的节目信息才能获取里面 拼接出字符串，所以验证这个机制是否正常，先识别一部剧或者一个电影，如果手动识别 没有节目信息证明负责api的那个 worker连通性不正常，tmdb没办法请求到图片。

#### imdb omdb tvdb 插件属于无效插件还会影响 tmdb 的拼接图片抓取地址，可以删掉。

#### 上面的注意都规避了，已经有了正确的节目文字信息，但图片还是占位图咋办，移除识别后刷新元数据+替换现有图像，因为有之前的错误图像地址缓存下来了。

#### 想加快更新出最新节目信息和图像，可以emby插件市场装 Episode Refresh 插件
